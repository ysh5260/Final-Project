							...
							<div class="tab-pane fade show active" id="Preview" role="tabpanel"
								...
								<div class="card-body px-0 pe-0"> 
									<ul>
										...
										<br>
										<li>내용 :
										<button class="btn btn-xxs btn-outline-primary" id="smsAutoBtn" style="float:right;">자동완성</button>
										</li>
							...
							<tbody>
								<c:forEach items="${smsList}" var="sms" varStatus="i">
									<tr>
										...
										<td>${sms.userName}</td>
										<td>${sms.phone}</td>
										<td style="text-overflow:ellipsis; overflow:hidden; white-space:nowrap; max-width:200px;text-align:left;">${sms.smsContent}</td>
										<td>${sms.smsDate}</td>
									</tr>
								</c:forEach>
							</tbody>
...
// 				alert("문자 전송이 완료되었습니다.");
	...
	$(document).on("click", "#smsAutoBtn",function(){
		console.log("자동완성버튼");
		$("#comment").text("대원대학교 등록금 납부 안내입니다. \n 납부 기간 : 2023.05.12.(금) ~ 2023.05.18.(목) 09:00 ~ 16:00 까지 \n 납부 기간 준수 바람");
	});
